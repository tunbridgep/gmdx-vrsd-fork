...
var(GMDX) const bool bDontRemoveOnMissionComplete;                                    //SARGE: Don't remove this ammo on mission completion.
...
function bool HandlePickupQuery( inventory Item )                               //RSD: Function to override Ammo.uc in Engine classes for adjusted ammo counts
{
	...
	{
        intj = player.LootAmmo(string(self.name),class<DeusExAmmo>(item.class),Ammo(item).AmmoAmount,true,false,false,true,false);
        ...
        else if (item.IsA('DeusExAmmo'))
        {
            ...
            if (player.bAlwaysShowReceivedItemsWindow)
                player.AddReceivedItem(string(self.name),item, intj, false);
            
            if (player.bAlwaysShowReceivedItemsWindow && player.bShowDeclinedInReceivedWindow)
                player.AddReceivedItem(string(self.name),item, Ammo(item).AmmoAmount, false, true);
        }
        return true;
	}
	if ( Inventory == None )
...
//	function Touch( actor Other )
	{
		...
		if( ValidTouch(Other) )
		{
            ...
            if (P != None)
            {
                i = P.LootAmmo(string(self.name),Self.Class,AmmoAmount,true,false);
                AmmoAmount -= i;
                if (AmmoAmount == 0)
                    bDestroy = true;
            }
