...
//log("  invItemTo   = " $ invItemTo);
    ...
    {
        if (invokeActor.IsA('UNATCOTroop') && invItemFrom.IsA('Ammo10mm'))
            Ammo10mm(invItemFrom).AmmoAmount = 13;
        else if (invokeActor.IsA('SamCarter') && invItemFrom.IsA('Ammo762mm'))
            Ammo762mm(invItemFrom).AmmoAmount = 60;
        else if (invokeActor.IsA('SamCarter') && invItemFrom.IsA('AmmoRocket'))
            AmmoRocket(invItemFrom).AmmoAmount = 1;

        //CyberP: Other bugged convos:
	...
	if ((invItemTo == None || (invItemTo.IsA('Pickup') && !Pickup(invItemTo).bCanHaveMultipleCopies)) && //RSD: added pickup stipulation so we don't gloss over inventory checks for pickups we already own!
	    ...
	    (DeusExPlayer(event.toActor).FindInventorySlot(invItemFrom, True) == False))
	{
		...
		if (bSpawnedItem)
			invItemFrom.Destroy();
        //if (invItemTo != none)    //CyberP:
        //    invItemTo.Destroy();  //CyberP:
		return nextAction;
	}
	...
	if (invItemTo != None)
	{
		...
		else
		{
			...
			{
			    if (invItemFrom != None)  //CyberP: bugfix
                    invItemFrom.Destroy();
				return nextAction;
            }
