...
function PlayerLogin(PlayerPawn P)
{
    ...
    if (fMusicHackTimer > 0 && DeusExPlayer(P) != None)
        DeusExPlayer(P).SoundVolumeHackFix();
    ...
    //Fix up song if it's not set.
    //if (info.SongOverride == None && p.Level.Song != None)
    //    info.SongOverride = p.Level.Song;
