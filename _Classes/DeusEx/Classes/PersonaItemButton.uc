...
var bool bAugmentationBtn;
var bool bColorSwitch;
var StaticInterlacedWindow winVisionLines;
var bool bMedBotCallPass;
// ----------------------------------------------------------------------
// InitWindow()
// ----------------------------------------------------------------------
...
function SelectButton(Bool bNewSelected)
{
	...
	if (bAugmentationBtn)
	PlaySound(Sound'GMDXSFX.Generic.publicterminalfrob', 0.75, 1.3);
	else
...
function Tick(float deltaTime)
{
   if (bAugmentationBtn)
   {
     if (bColorSwitch)
     {
     colSelectionBorder.R += deltaTime*150;
     colSelectionBorder.G += deltaTime*150;
     colSelectionBorder.B += deltaTime*150;
     if (colSelectionBorder.R >= 240)
        bColorSwitch = False;
     }
     else
     {
     ...
     colSelectionBorder.G -= deltaTime*150;
     colSelectionBorder.B -= deltaTime*150;
     if (colSelectionBorder.R <= 60)
        bColorSwitch = True;
     }
     if(winVisionLines == None && !bMedBotCallPass){
			...
			winVisionLines.SetSize(iconPosWidth, iconPosHeight);
			winVisionLines.linespeed=10;
			winVisionLines.RandomizeStatic();
			winVisionLines.SetPos(1, 1);
			winVisionLines.SetBackgroundStyle(DSTY_Translucent);
