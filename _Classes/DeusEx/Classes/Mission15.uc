...
function Timer()
{
	...
	else if (localURL == "15_AREA51_ENTRANCE")
	{
		...
		//CyberP: MJ12 ambush that ghosters/non-lethalers do not have to face.
		if (flags.GetBool('ambushVariable'))
		{
			count = 0;

			// count the living
			foreach AllActors(class'HumanMilitary', hm)
			    if (hm.bInWorld) //don't count those that are waiting to be spawned in via alarm
				     count++;

			// add the unconscious ones to the not dead count, any carc will do aside from animals since we can't
			// specify classes in packages and there are no K.O-ables in this level besides enemies.
			foreach AllActors(class'DeusExCarcass', carc)
			{
				if ((carc.KillerBindName == "JCDenton") && (!carc.bAnimalCarcass) && (carc.bNotDead))
					count++;
			}

			// if there are less than 8 among the living, setup the ambush
			if (count < 8)
			{
			    foreach AllActors(class'MJ12Troop', mjt, 'MJ12Squad')
			    {
			         mjt.EnterWorld();
			         if (Player.bHardCoreMode)
			             mjt.EnemyTimeout = 20.000000;
			         else
                         mjt.EnemyTimeout = 11.000000;
			    }
            }
            flags.SetBool('ambushVariable', false);
		}
	}
	...
	else if (localURL == "15_AREA51_PAGE")
	{
		...
		if (!player.bHardCoreMode && !flags.GetBool('RSD_nonHardcoreRemoveForceFields'))
		{
        	ForEach AllActors(class'DeusExMover', M, 'shieldgen')
			{
        		//M.Trigger(self, None);
        		M.LowKeyDestroy(none);
			}
			flags.SetBool('RSD_nonHardcoreRemoveForceFields',True);
		}
	}
}
