...
function PreTravel()
{
    if (Player.bGMDXNewGame)
    {
       Player.ResetPlayer();  //CyberP/Totalitarian: goddamn it. New game inventory bug training-exclusive fix
       Player.bGMDXNewGame=False;
    }
	Super.PreTravel();
}
...
function Timer()
{
	...
	else if (localURL == "00_TRAININGFINAL")
	{
		...
		if (flags.GetBool('00_Training_Fin'))
		{
            /*if (player != none)
            {
            ...
            flags.SetBool('00_Training_Fin', False);
		}
	}
}
