...
function FirstFrame()
{
	  ...
	  if (flags.GetBool('QuickLetPlayerIn'))
			...
			!flags.GetBool('MS_ChenTeleported'))
		{
			foreach AllActors(class'ScriptedPawn', pawn)
			{
				...
				{
				    MaxChen(pawn).HomeTag = '';
				    MaxChen(pawn).HomeExtent = 4000;
	...
	else if (localURL == "06_HONGKONG_WANCHAI_MARKET")
	{
			...
			!flags.GetBool('TriadCeremony_Played'))
		{
			...
			flags.SetBool('CeremonyReadyToBegin', True);
		}
			...
			!flags.GetBool('MS_DoorsBlown'))
		{
			...
			else
			{
				...
				if (fireTime > 3)
				{
					...
					//fireTime = 0;
					foreach AllActors(class'DeusExMover', M, 'Blast_doors') //CyberP: instead if Jock misses just blow the doors anyway, as a failsafe.
				        if (!M.bDestroyed)
				        {
				          M.bDrawExplosion = True;
				          M.TakeDamage(400,none,vect(0,0,0),vect(0,0,0),'Exploded');
				        }
				}
			}
		}
