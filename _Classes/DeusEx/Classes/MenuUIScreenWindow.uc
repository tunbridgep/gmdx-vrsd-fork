...
function SaveSettings()
{
	...
	while(btnChoice != None)
	{
		if (btnChoice.IsA('MenuUIChoice')) //gmdx kill save for xhair if lasing DTS as exit menu saves
		{
		   if (MenuUIChoice(btnChoice).IsA('MenuChoice_Crosshairs'))
		   {
		      if ((player!=none)&&
            (player.Weapon!=none))
            {
             if (!(player.Weapon.isA('WeaponNanoSword')&&DeusExWeapon(player.Weapon).bLasing))
               MenuUIChoice(btnChoice).SaveSetting();
            }else MenuUIChoice(btnChoice).SaveSetting();
      	}
         else MenuUIChoice(btnChoice).SaveSetting();
      }
		btnChoice = btnChoice.GetLowerSibling();
