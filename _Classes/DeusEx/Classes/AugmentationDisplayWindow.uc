...
//SARGE: Added Autoaim aug
var bool bAutoaimActive;
var ScriptedPawn autoaimTarget;
var ScriptedPawn autoaimPotentialTarget;
var bool autoaimValidWeapon;
...
function Actor TraceLOS(float checkDist, out vector HitLocation)
{
   ...
   /* if (Player.AugmentationSystem.GetAugLevelValue(class'AugIFF') >= 2.0)
	...
	{
		if ((target.IsA('Pawn') && !Pawn(target).IsInState('Dying')) || target.IsA('DeusExDecoration') || target.IsA('ThrownProjectile') ||
            target.IsA('DeusExWeapon') || (target.IsA('DeusExMover') && DeusExMover(target).bBreakable) || target.IsA('DeusExPickup'))
		{
...
function Tick(float deltaTime)
{
	...
	if (bVisionActive)
	{
          //player.HeadRegion.Zone.ViewFog.Y = sin(player.FrobTime*0.4);
          if (lite != None)
          {
             lite.SetLocation(Player.Location);
          }
          if(winVisionLines == None){
			...
			if (lite == None)
			{
			    lite = Player.Spawn(class'ConLight');
			    if (lite != None)
			    {
			    lite.LightRadius=64;
                lite.LightBrightness=16;
                lite.LightSaturation=255;
                lite.LightCone=128;
                lite.LightType = LT_Steady;
                }
			}
			//player.Level.Brightness = 6.000000;
			//if (player.RendMap != 6)
			//    Player.RendMap = 6;
			...
			if (CurrentBrightness >= 0.8)
                ...
                player.ConsoleCommand("set" @ "ini:Engine.Engine.ViewportManager Brightness" @ 0.8);
            player.ConsoleCommand("FLUSH"); */
			//player.changeZoneLight(True);
			//Player.ClientAdjustGlow(10.000000, vect(0,0,0));
			}
	}
	else if(!bVisionActive)
    {
		if(winVisionLines != None){
            ...
            //player.Level.Brightness = 1.000000;
			//player.ConsoleCommand("FLUSH");
			//player.ConsoleCommand("set" @ "ini:Engine.Engine.ViewportManager Brightness" @ CurrentBrightness);
            //player.ConsoleCommand("FLUSH");
			//player.changeZoneLight(false);
			//if (player.RendMap != 5)
			//    Player.RendMap = 5;
			//Player.ClientAdjustGlow(0.000000, vect(0,0,0));
		}
...
function PostDrawWindow(GC gc)
{
    ...
    //SARGE: Added new targeting aug display
    if (bAutoaimActive)
		DrawAutoaimAugmentation(gc);
...
// ----------------------------------------------------------------------
// DrawAutoaimAugmentation()
// ----------------------------------------------------------------------

function DrawAutoaimAugmentation(GC gc)
{
    local int i;
    local float W,H,TLX,TLY,CX,CY,targetCX,targetCY;
    local string str, str2;
    local ScriptedPawn targ;
    ...
    DrawDropShadowBox(gc,TLX,TLY,W,H);
    

    if (autoaimValidWeapon)
    {
        str = "Tracking Targets";
        gc.SetTextColorRGB(0,255,0);
    }
    else
    {
        str = "No Compatible Weapon Found";
        gc.SetTextColorRGB(255,0,0);
    }

    gc.GetTextExtent(0, w, h, str);
    gc.DrawText(CX, CY, w, h, str);


    if (autoaimTarget != None)
    {
        targ = autoaimTarget;
        str2 = autoaimTarget.UnfamiliarName;
        gc.SetTextColorRGB(0,160,16);
    }
    else if (autoaimPotentialTarget != None)
    {
        targ = autoaimPotentialTarget;
        str2 = "Acquiring Target";
        gc.SetTextColorRGB(150,160,16);
    }

    if (targ != None)
    {
		if (ConvertVectorToCoordinates(targ.Location, targetCX, targetCY))
        {
            ...
            gc.DrawText(targetCX, targetCY, w, h, str2);
            gc.SetTextColor(colHeaderText);

            //Draw line
			gc.SetTileColorRGB(255,0,0);
			Interpolate(gc, width/2, height/2, targetCX, targetCY, 64);
			gc.SetTileColor(colHeaderText);
        }
    }
}
...
// ----------------------------------------------------------------------
// SARGE: DrawTargetAugmentation changed to DrawIFF, Targeting is now done elsewhere()
// ----------------------------------------------------------------------

function DrawIFF(GC gc)
{
...
function DrawVisionAugmentation(GC gc)
{
	...
	boxW = width;          //all were divided by 2
	boxH = height;
	boxCX = width;
	boxCY = height;
	boxTLX = boxCX - boxW;
	...
	if (visionLevel >= 1)
	{
		...
		if (player.Level.Netmode == NM_Standalone)
		{
			...
			gc.DrawText(320, 6, w, h, msgIRAmpActive);
		}
	}
	...
	if (player.Level.Netmode == NM_Standalone && visionLevel < 1)
	{
		gc.SetStyle(DSTY_Modulated);
		gc.DrawPattern(boxTLX, boxTLY, boxW, boxH, 0, 0, Texture'SolidGreen');
		gc.DrawPattern(boxTLX, boxTLY, boxW, boxH, 0, 0, Texture'SolidGreen');
	...
	if (player.Level.Netmode == NM_Standalone)
	{
		...
		gc.DrawText(192, 6, w, h, msgLightAmpActive);
	}
}
