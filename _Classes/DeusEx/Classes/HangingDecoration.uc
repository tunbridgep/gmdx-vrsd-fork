...
function CalculateHit(vector Loc, vector Vel)
{
	...
	{
	newsway.Pitch = -Cos(ang) * VSize(Vel) * 4;
	newsway.Roll = Sin(ang) * VSize(Vel) * 4;
	}
	newsway.Yaw = 0;
...
local vector frobVelocity;

	Super.Frob(Frobber, frobWith);

    player = DeusExPlayer(GetPlayerPawn());

    if (IsA('HangingChicken') || IsA('HKHangingPig'))
    {
	if (bUsing)
		return;
	...
	SetTimer(7.0, False);
	bUsing = True;
	frobVelocity.X = FRand()*350;
	frobVelocity.Y = FRand()*120;
	frobVelocity.Z = 0;

	CalculateHit(player.Location,frobVelocity);
	bSwaying = True;
	if (!bSwaying)
		swayTimer = 0;
	}
