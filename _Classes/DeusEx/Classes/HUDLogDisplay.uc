...
var bool bPlayingLog;                                                           //RSD: Lazy and possibly redundant var to decide if we should unhide the window when closing realtime UI
...
event Tick(float deltaSeconds)
{
		 ...
		 ((player.conPlay != None) && (player.conPlay.GetDisplayMode() == DM_FirstPerson))))
	{
		if (( lastLogMsg > 0.0 ) && ( lastLogMsg + deltaSeconds > displayTime ))
		{
			...
			HideLogWindow();                                                    //RSD: Was Hide(), new shell function so we can set bPlayingLog properly
...
function ShowLogWindow()                                                        //RSD: New shell for Show() so we can set bPlayingLog properly
{
	bPlayingLog = true;
	Show();
}

function HideLogWindow()                                                        //RSD: New shell for Hide() so we can set bPlayingLog properly
{
	bPlayingLog = false;
	Hide();
}
