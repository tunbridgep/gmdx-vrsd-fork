	...
	optional Bool bNoPause)
{
    ...
    //CyberP: hide the belt with real ui and browsing interface
    if (hud != None && player != none)
        if (player.bHardCoreMode || player.bRealUI)
        {
             hud.belt.SetVisibility(false);
             if (hud.msgLog != none)
                 hud.msgLog.SetVisibility(false);                               //RSD: Also hide the log window
             if (hud.infolink != none)
                 hud.infolink.SetVisibility(false);                             //RSD: Also hide the infolink window
             if (hud.conWindow != none)
                 hud.conWindow.SetVisibility(false);                            //RSD: Also hide the con window
             if (hud.barkDisplay != none)
                 hud.barkDisplay.SetVisibility(false);                          //RSD: Also hide the bark window (???)
             if (hud.startDisplay != none)
	...
	optional Bool bNoUnpause)
{
	...
	if ( winCount > 0 )
	{
		...
		if (oldWindow.IsA('PersonaScreenInventory'))                            //RSD: Hack fix to reset item orientation if we're dragging while closing the window
			PersonaScreenInventory(oldWindow).cancelButtonRotation();
		oldWindow.Destroy();
